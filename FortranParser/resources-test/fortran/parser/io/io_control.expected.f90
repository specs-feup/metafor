PROGRAM IO_CONTROL_SPECS
    ...
    ! -----------------------------------------------------------------
    WRITE(UNIT=u_seq, FMT=*, DECIMAL="comma", SIGN="plus", ROUND="up", DELIM="quote", IOSTAT=ios, IOMSG=io_message)


    ! -----------------------------------------------------------------
    ! 2. Namelist Specifier (NML)
    ! -----------------------------------------------------------------
    WRITE(UNIT=u_seq, NML=my_namelist, IOSTAT=ios)
    ...
    ! 4. Direct Access (REC)
    ! -----------------------------------------------------------------
    WRITE(UNIT=u_dir, REC=1, IOSTAT=ios) val, x


    ! -----------------------------------------------------------------
    ! 5. Stream Access (POS)
    ! -----------------------------------------------------------------
    WRITE(UNIT=u_stream, POS=10, IOSTAT=ios) "Stream Header"

    ! -----------------------------------------------------------------
    ! 6. Asynchronous Non-Blocking I/O (ASYNCHRONOUS, ID)
    ! -----------------------------------------------------------------
    WRITE(UNIT=u_seq, FMT="(I5)", ASYNCHRONOUS="yes", ID=async_id, IOSTAT=ios) val
    ...
    ! -----------------------------------------------------------------
    READ(UNIT=u_seq, FMT="(I5)", BLANK="zero", END=200, ERR=900, IOSTAT=ios) val
